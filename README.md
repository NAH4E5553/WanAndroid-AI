...
专题页面已完成本地实现：左侧只显示一级分类，点击选中及圆角背景参考 CoolMallKotlin；右侧顶部仅展示接口返回的真实二级分类标签，支持点击与文章区域横滑切换，到底不会切换下一个一级分类。默认加载第一个二级分类，以子分类 cid 请求文章；分类按真实 ID 独立保存分页和滚动位置，阅读/Tab 返回保留状态。二级标签离开第一个真实分类后侧栏左滑淡出、右侧扩展全宽，回到第一个真实分类时反向恢复，保留两侧滚动位置。10 个专题单元测试和 19 个 Android 15 固定响应专题 UI 测试通过；专题真机验收待完成。

当前完整验证入口已通过：Debug 构建、109 个单元测试、Spotless、架构检查及 Lint。
