...
当前完整验证入口已通过：Debug 构建、99 个单元测试、Spotless、架构检查及 Lint。

界面上的“待接入”是明确的占位状态，不是网络加载失败或真实数据。
专题状态、Cookie 登录、收藏、阅读历史页面和正文缓存尚未完成。在线阅读只恢复安全当前 URL，不保证重建后的完整网页历史、表单或滚动位置；第三方网页暗色效果受页面与 WebView 能力限制。搜索进程恢复会重新请求已提交关键词的首屏，不保证恢复所有已加载分页。
...
完成本次搜索真机验收与 PR 交付，再推进剩余专题、登录/收藏及本地数据阶段。历史和离线缓存按独立阶段实施；不要将占位页面算作已完成的功能。
